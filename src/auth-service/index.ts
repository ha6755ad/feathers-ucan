...
export const genAuthService = (ParentClass:any, options?: AuthServiceOptions) => {
    class AuthService extends ParentClass {
        constructor(app: any, configKey = 'authentication', opts = {}) {
            super(app, configKey, opts)
            this.app = app;
        };

        async create(data: AuthenticationRequest, params?: AuthenticationParams) {
            const NotAuth = options?.NotAuthenticated || NotAuthError;
